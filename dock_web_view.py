...
jgs    \  `-"`      `"-`   /
    ...
    def more_function(self, set_text, search_text=None):
        # 1 •  Nếu người dùng bôi đen văn bản → lấy ngay làm last_text
        if search_text is not None:
            self.set_last_text(search_text)
            # Lưu lại note hiện hành để trích mọi field
            if hasattr(mw, "reviewer") and hasattr(mw.reviewer, "card"):
                self.last_card_note = mw.reviewer.card.note()

        # 2 •  Phải có self.last_text mới thực hiện
        if self.last_text is None:
            return

        config = mw.addonManager.getConfig(__name__)
        more_info_list = config[set_text]              # list prompt của nút
        more_info      = random.choice(more_info_list) # chọn ngẫu nhiên

        # 3 •  Tạo dict {FieldName: Value} (nếu biết note)
        dict_fields = {}
        if self.last_card_note:
            dict_fields = {
                fld["name"]: self.last_card_note[fld["name"]]
                for fld in self.last_card_note.note_type()["flds"]
            }

        # 4 •  Thay thế placeholder
        try:
            prompt_text = more_info.format_map(dict_fields)   # dùng {Word}, {Reading}…
        except (KeyError, ValueError):
            # Fallback cho prompt kiểu “{}” cũ
            if "{}" in more_info:
                prompt_text = more_info.format(
                    SYMBOL[CHOICE_SYMBOL][0] + self.last_text + SYMBOL[CHOICE_SYMBOL][1]
                )
            else:
                prompt_text = self.last_text + more_info
    ...
    def load_and_interact(self, card: Optional[Card] = None, *args, **kwargs):
        # 1 •  Dock ẩn ⇒ thôi
        if not self.isVisible():
            return

        # 2 •  Giới hạn gọi lại trong 1 s
        ...
        # 3 •  Lấy card hiện hành
        if card is None and self.last_card is not None:
            card = self.last_card
        if card is None:
            return
        self.last_card = card
        ...
        # 4 •  Bỏ qua note-type nằm trong Exclude
        if self.is_excluded_note_type(config, note_type):
            return

        # 5 •  Field ưu tiên (thường là “Word” / “Front” …)
        first_field_name = self.get_priority_field_name(config, note_type)

        # 6 •  Chọn prompt (random_prompt, more_info …)
        default_prompt = config.get("default_prompt", "random_prompt")
        if default_prompt not in config:
            default_prompt = "random_prompt"
        selected_prompt = random.choice(config[default_prompt])

        # 7 •  Tạo dict {FieldName: Value} cho toàn bộ field
        note_fields = {fld["name"]: note[fld["name"]] for fld in note_type["flds"]}

        # 8 •  Giá trị ưu tiên (dùng cho fallback “{}”)
        note_field_text = note[first_field_name]
        self.set_last_text(note_field_text)

        # 9 •  Thay thế placeholder
        try:
            # Thay mọi {FieldName} bằng giá trị tương ứng
            prompt_text = selected_prompt.format_map(note_fields)
        except (KeyError, ValueError):
            # Prompt không khớp hết field → fallback cơ chế cũ
            if "{}" in selected_prompt:
                prompt_text = selected_prompt.format(
                    SYMBOL[CHOICE_SYMBOL][0] + note_field_text + SYMBOL[CHOICE_SYMBOL][1]
                )
            else:
                prompt_text = note_field_text + selected_prompt

        # 10 •  Gửi sang hàm xử lý tiếp
    ...
    def handle_load_finished(self, prompt_text:str, click=False):
        ...
        #     class_name = 'input[name="query"]' #".c92459f0"
