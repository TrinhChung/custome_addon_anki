...
def getSpeakerInfo(speaker_uuid):
    try:
        speakers_response = requests.get(
            "http://127.0.0.1:50021/speaker_info?speaker_uuid=" + str(speaker_uuid),
            timeout=5,
        )
        if speakers_response.status_code == 200:
            print(speakers_response.content)
...
class MyDialog(qt.QDialog):
    def __init__(self, browser, parent=None) -> None:
        ...
        for field in common_fields:
            if last_source_field is None:
                if (
                    "expression" == field.lower()
                    or "sentence" == field.lower()
                    or "front" == field.lower()
                ):
        ...
        if speaker_json is None:
            layout.addWidget(
                qt.QLabel(
                    "VOICEVOX service was unable to get speakers list. Please make sure the VOICEVOX service is running and reopen this dialog"
                )
            )
        ...
        def update_speaker_style_combo_box():
            speaker_name = self.speaker_combo.itemText(
                self.speaker_combo.currentIndex()
            )
            speaker = next(
                (x for x in self.speakers if x[0] == speaker_name), None
            )  # grab the first speaker with this name
            if speaker is None:
        ...
        i = 0
        for style_item in [
            self.style_combo.itemText(i) for i in range(self.style_combo.count())
        ]:
        ...
        self.style_combo.setCurrentIndex(
            style_combo_index
        )  # NOTE: The previous style should probably be stored as a tuple with the speaker, but this is good enough. IE. Person A style X is not the same as Person B style X
        ...
        self.preview_voice_button_actual.setToolTip(
            "Preview the source field\n\nRepeated clicks cycle through each note."
        )
        ...
        default_template = (
            config.get("filename_template") or "VOICEVOX_{{speaker}}_{{style}}_{{uid}}"
        )
        ...
        self.uid_warning_label.setToolTip(
            "Warning: Without {{uid}}, you might get file name collisions. Each generated audio must have a globally unique name."
        )
        # Use some built-in icon or text: exclamation triangle
        self.uid_warning_label.setPixmap(
            self.style()
            .standardIcon(qt.QStyle.StandardPixmap.SP_MessageBoxWarning)
            .pixmap(16, 16)
        )
        ...
        pitch_slider.valueChanged.connect(
            update_slider(
                pitch_slider, pitch_label, "pitch_slider_value", "Pitch scale"
            )
        )
        ...
        speed_slider.valueChanged.connect(
            update_slider(
                speed_slider, speed_label, "speed_slider_value", "Speed scale"
            )
        )
        ...
        intonation_slider.valueChanged.connect(
            update_slider(
                intonation_slider,
                intonation_label,
                "intonation_slider_value",
                "Intonation scale",
            )
        )
        ...
        initial_silence_slider.setValue(
            config.get("initial_silence_slider_value") or 10
        )

        initial_silence_label = QLabel(
            f"Initial silence scale {initial_silence_slider.value() / 100}"
        )

        initial_silence_slider.valueChanged.connect(
            update_slider(
                initial_silence_slider,
                initial_silence_label,
                "initial_silence_slider_value",
                "Initial silence scale",
            )
        )
        ...
        final_silence_label = QLabel(
            f"Final silence length {final_silence_slider.value() / 100}"
        )

        final_silence_slider.valueChanged.connect(
            update_slider(
                final_silence_slider,
                final_silence_label,
                "final_silence_slider_value",
                "Final silence scale",
            )
        )
    ...
    def getNoteTextAndSpeaker(self, note_id):
        (speaker_index, speaker, style_info) = getSpeaker(
            self.speakers, self.speaker_combo, self.style_combo
        )
...
def GenerateAudioQuery(text_and_speaker_index_tuple, config):
    try:
        ...
        audio_query_response = requests.post(
            "http://127.0.0.1:50021/audio_query?speaker="
            + str(speaker_index)
            + "&text="
        ...
        if audio_query_response.status_code != 200:
            raise Exception(
                f"Unable to generate audio for the following text: `{text}`. Response code was {audio_query_response.status_code}\nResponse:{audio_query_response.text}"
            )
...
def MultiSynthesizeAudio(audio_queries, speaker_index):  # NOTE: This returns a zip
    for q in audio_queries:
        if q is None:
            raise Exception(
                "MultiSynthesizeAudio recieved an audio query that was None"
            )
    # Create json array of queries
    combined = b"[" + b",".join(audio_queries) + b"]"

    synthesis_response = requests.post(
        "http://127.0.0.1:50021/multi_synthesis?speaker=" + str(speaker_index),
        data=combined,
    )
    if synthesis_response.status_code != 200:
        return None
...
def onVoicevoxOptionSelected(browser):
    ...
    if not voicevox_exists:
        QMessageBox.critical(
            mw,
            "Error",
            f"VOICEVOX service is not running. Navigate to your VOICEVOX install and run 'run.exe'. You can download VOICEVOX from https://voicevox.hiroshiba.jp/ if you do not have it installed",
        )
        return
    ...
    if dialog.exec():
        (speaker_index, speaker, style_info) = getSpeaker(
            dialog.speakers, dialog.speaker_combo, dialog.style_combo
        )
        if speaker_index is None:
        ...
        style_combo_text = dialog.style_combo.itemText(
            dialog.style_combo.currentIndex()
        )
        ...
        def updateProgress(notes_so_far, total_notes, bottom_text=""):
            progress_text.setText(
                f"Generating Audio {notes_so_far}/{total_notes}\n{bottom_text}"
            )
        ...
        filename_template = config.get(
            "filename_template", "VOICEVOX_{{speaker}}_{{style}}_{{uid}}"
        )

        for note_chunk in note_chunks:
            note_text_and_speakers = map(dialog.getNoteTextAndSpeaker, note_chunk)
            updateProgress(
                notes_so_far, total_notes, f"Audio Query: {0}/{len(note_chunk)}"
            )
            query_count = 0
            ...
            def GenerateQueryAndUpdateProgress(x, query_count):
                updateProgress(
                    notes_so_far,
                    total_notes,
                    f"Audio Query: {query_count}/{len(note_chunk)}",
                )
                query_count += 1
            ...
            audio_queries = list(
                map(
                    lambda note: GenerateQueryAndUpdateProgress(note, query_count),
                    note_text_and_speakers,
                )
            )
            ...
            updateProgress(
                notes_so_far,
                total_notes,
                f"Synthesizing Audio {notes_so_far} to {min(notes_so_far+CHUNK_SIZE, total_notes)}",
            )
            ...
            with zipfile.ZipFile(
                io.BytesIO(zip_bytes), "r", zipfile.ZIP_STORED
            ) as wavs_zip:
                for name in wavs_zip.namelist():
                    updateProgress(
                        notes_so_far,
                        total_notes,
                        f"Converting Audio: {zip_counter}/{len(note_chunk)}",
                    )
                    zip_counter += 1
                    audio_data = wavs_zip.read(name)
                    chunk_note_index = (
                        int(name.replace(".wav", "")) - 1
                    )  # Starts at 001.wav, this converts to 0 index
                    ...
                    raw_filename = parse_filename_template(
                        filename_template, placeholders
                    )
